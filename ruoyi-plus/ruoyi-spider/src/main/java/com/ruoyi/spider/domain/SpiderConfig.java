...
public class SpiderConfig extends BaseEntity
{
    ...
    private Integer isJson=0;

    private Integer isSelenium=0;

    private String spiderHighSetting;

    private String userDefinePipeline;

    private String missionFinish;//任务结束的回调方法
    private String listExtractBy;//列表提取规则
    /**
            ...
            if(arr!=null) {
                for (String s : arr) {
                    if (StringUtils.isNotEmpty(s) && isURL(s)) {
                        if(!entryUrlsList.contains(s.trim())){
                            entryUrlsList.add(s.trim());
                        }
                    }
                }
            }else{
                if(isURL(entryUrls)){
                    if(!entryUrlsList.contains(entryUrls.trim())) {
    ...
    public SpiderConfig setIsJson(Integer isJson) {
        this.isJson = isJson;
        return this;
    }
    public SpiderConfig setIsSelenium(Integer isSelenium) {
