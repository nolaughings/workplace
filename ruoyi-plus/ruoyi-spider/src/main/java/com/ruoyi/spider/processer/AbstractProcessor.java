...
import com.ruoyi.spider.resolver.JsonResolver;
...
public abstract class AbstractProcessor implements PageProcessor {
     ...
     * @return 返回列表
     */
    public abstract CopyOnWriteArrayList<LinkedHashMap<String, Object>> execute();

    @Override
    public void process(Page page) {
        Resolver resolver = new DefaultResolver();
        if (config.getIsJson()==1){
     ...
     * @param datas 数据集合
     */
    final void processData(ResultItems resultItems, List<LinkedHashMap<String, Object>> datas, MyConfigurableSpider spider) {
        if (null == spider) {
            return;
        }
        ...
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            tempMap.put(entry.getKey(),entry.getValue()==null?"":entry.getValue());
        }
        datas.add(tempMap);
    }
